const ProblemShimmer=()=>{
  return(
    <div className="h-[90vh] w-full text-center">Loading...</div>
  )
}

const Problem = ({ problem }) => {
    return !problem?<ProblemShimmer/>:(
      <div className="h-[92vh] border rounded-xl mb-0 bg-white">
        <div className="font-semibold flex border-b gap-5 min-h-10 ">
          <a className="ml-2 border-b border-black mt-auto">Question</a>
          <a className="mt-auto">Solution</a>
          <a className="mt-auto">Submissions</a>
        </div>
        <div className="h-[calc(92vh-45px)] overflow-y-scroll">
          <h1 className="text-2xl p-4 font-semibold ">{problem?.name}</h1>
          <div
            className="m-2"
...
//       <h1 className="text-2xl mb-4 font-semibold">2402.Meeting-rooms-iii</h1>
//       <p>
